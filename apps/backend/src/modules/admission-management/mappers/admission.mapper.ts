  ...
  static toResponseDto(record: any): ApplicationResponseDto {
    ...
    const assessment: AssessmentResponseDto | null = record.application_assessments
      ? {
          ...
          maximum_marks: record.application_assessments.maximum_marks ? Number(record.application_assessments.maximum_marks) : null,
          marks_obtained: record.application_assessments.marks_obtained ? Number(record.application_assessments.marks_obtained) : null,
          percentage: record.application_assessments.percentage ? Number(record.application_assessments.percentage) : null,
    ...
    const decision: DecisionResponseDto | null = record.admission_decisions
      ? {
          ...
          offer_expiry_date: record.admission_decisions.offer_expiry_date ? new Date(record.admission_decisions.offer_expiry_date).toISOString() : null,
          waitlist_position: record.admission_decisions.waitlist_position || null,
          scholarship_percentage: record.admission_decisions.scholarship_percentage ? Number(record.admission_decisions.scholarship_percentage) : null,
        }
      : null;
    ...
    const payment: FeePaymentResponseDto | null = record.admission_fee_payments
      ? {
          ...
          payment_date: record.admission_fee_payments.payment_date ? new Date(record.admission_fee_payments.payment_date).toISOString() : null,
    ...
    return {
      ...
      application_date: record.application_date ? new Date(record.application_date).toISOString() : new Date().toISOString(),
