...
import {
  ...
  type WorkflowEvent,
} from '../../services/application/ApplicationWorkflowOrchestrator';
...
export class ApplicationController {
  ...
  private async checkFlags(req: Request) {
    ...
    const isCrmActive = await this.flagService.isEnabled(
      'admission',
      'admission_crm',
      envMode,
      schoolId,
    );
    const isAppActive = await this.flagService.isEnabled(
      'admission',
  ...
  private async enforceAccess(req: Request, applicationId: string): Promise<void> {
    ...
    if (
      !user.permissions?.includes('admission.application.view') &&
  ...
  public create = async (req: Request, res: Response) => {
    try {
      await this.checkFlags(req);
      const schoolId =
        req.context?.user?.school_id || req.body.school_id || req.body.org_id || 'school-main';
      let academicYearId =
  ...
  public patchProfile = async (req: Request, res: Response) => {
    try {
      ...
      const expectedUpdatedAt =
        (req.headers['x-expected-updated-at'] as string) || req.body.expected_updated_at;
      ...
      const correlationId = req.headers['x-correlation-id'] as string;
      await this.draftService.patchDraftSection(
        id,
        'profile',
        req.body,
        expectedUpdatedAt,
  ...
  public submit = async (req: Request, res: Response) => {
    try {
      ...
      const data = await this.appService.submitApplication(
        id,
        req.body,
        role,
        performedBy,
  ...
  public transition = async (req: Request, res: Response) => {
    try {
      ...
      if (event) {
        const data = await this.workflowOrchestrator.publish(event as WorkflowEvent, id, {
          userId: performedBy,
          role,
          correlationId,
          notes,
  ...
  public reject = async (req: Request, res: Response) => {
    try {
      ...
      const data = await this.appService.rejectApplication(
        id,
        performedBy,
        rejectionReason,
        role,
  ...
  public verifyDocuments = async (req: Request, res: Response) => {
    try {
      ...
      const data = await this.appService.verifyDocuments(
        id,
        performedBy,
        remark ?? '',
        role,
