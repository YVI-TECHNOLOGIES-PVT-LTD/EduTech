        ...
        if (candidates.length > 0) {
          if (inputFirstName) {
            const matched = candidates.find(
              (c) => (c.student_first_name || '').trim().toLowerCase() === inputFirstName,
            );
            if (matched) {
              lead = matched;
            }
            ...
            const draftApp = await prisma.admissions_applications.findFirst({
              where: { lead_id: candidate.lead_id, status: 'draft' as any },
            });
            if (draftApp) {
              lead = candidate;
            }
