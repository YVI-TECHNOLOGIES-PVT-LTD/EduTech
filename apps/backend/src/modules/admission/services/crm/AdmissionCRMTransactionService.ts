...
export class AdmissionCRMTransactionService extends BaseService {
   ...
   */
  public async convertEnquiryToLead(
    enquiryId: string,
    leadId: string,
    ...
    const sqlQueries: string[] = [
      `UPDATE public.leads SET stage = 'qualified', updated_at = NOW() WHERE lead_id = '${enquiryId}'`,
    ...
    this.logInfo(
      `Successfully converted enquiry ${enquiryId} to lead ${leadId} inside database transaction.`,
      correlationId,
    );
  }
}
