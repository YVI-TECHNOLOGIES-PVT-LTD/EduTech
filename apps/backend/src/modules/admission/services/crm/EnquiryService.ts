...
import {
  mapEnquiryToApiRecord,
  resolveAssignmentHistory,
  resolveCounselorNames,
} from './CrmRecordMapper';
...
export class EnquiryService extends BaseService {
  ...
  public async convertToLead(
    ...
    if (existingLead) {
      ...
      await this.transactionService.convertEnquiryToLead(
        enquiryId,
        leadId,
        correlationId,
        counselorId,
  ...
  public async convertToApplication(
    ...
    if (!enquiry.dateOfBirth) {
      throw new ValidationError(
        'Date of birth is required on the inquiry before converting to an application',
      );
    }
  ...
  public async checkDuplicates(enquiryData: any): Promise<{
    ...
    let status: 'no_duplicate' | 'potentials_found' | 'exact_match' | 'merge_candidate' =
      'potentials_found';
