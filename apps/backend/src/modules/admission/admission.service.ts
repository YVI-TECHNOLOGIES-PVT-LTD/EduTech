...
export class AdmissionService {
  ...
  static async getLeadDashboard(schoolId: string) {
  ...
  static async getLeadPipeline(schoolId: string) {
    const { data: leads, error } = await supabase
      .from('leads')
      .select('status')
      .eq('school_id', schoolId);
  ...
  static async createLead(schoolId: string, leadData: any) {
  ...
  static async createApplication(schoolId: string, appData: any) {
    const { data, error } = await supabase
      .from('admissions')
      .insert({
        school_id: schoolId,
  ...
  static async decideLogin(id: string, adminId: string, status: string, reason?: string) {
  ...
  static async submitPayment(id: string, userId: string, details: any) {
    const { data, error } = await supabase
      .from('admissions')
      .update({
  ...
  static async verifyPayment(id: string, userId: string, status: string, remarks?: string) {
    const { data, error } = await supabase
      .from('admissions')
      .update({
  ...
  static async uploadDocument(id: string, type: string, url: string) {
    const { data, error } = await supabase
  ...
  static async initializeBilling(admissionId: string, userId: string, fee_structure_ids: string[]) {
    return {
      success: true,
      admissionId,
      fee_structure_ids,
