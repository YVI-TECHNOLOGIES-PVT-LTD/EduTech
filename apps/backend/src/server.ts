...
import { redisConnectionManager } from './cache/redis.client';
  ...
  try {
    ...
    if (mappings.length > 0) {
      ...
      if (mappingError) {
        ...
        logger.info(
          `[RBAC Self-Healing] Successfully synchronized ${mappings.length} role-permission mappings.`,
        );
      }
    }
  } catch (e: any) {
...
const server = app.listen(Number(PORT), () => {
  ...
  // Connect to Redis if CACHE_PROVIDER=redis (no-op otherwise). Never blocks
  // or crashes startup — CacheService keeps serving from InMemoryCacheAdapter
  // until/unless Redis becomes available.
