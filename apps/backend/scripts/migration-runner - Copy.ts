  ...
  public async run(): Promise<void> {
    ...
    if (client) {
      try {
        ...
        logger.warn(
          `Could not connect to database at '${this.config.connectionString}': ${err.message}`,
        );
        logger.warn('Proceeding in static validation / dry-run inspection mode...');
      }
    } else {
      logger.info(
        'PostgreSQL driver (pg) not dynamically loaded. Running in static validation mode.',
      );
    }
    ...
    logger.info(
      `Detected ${allMigrations.length} total migrations. Filtered ${pendingMigrations.length} to evaluate.`,
    );
    ...
    if (this.options.dryRun) {
      for (const migration of pendingMigrations) {
        ...
        if (!valResult.valid) {
          ...
          logger.success(
            `Validation Passed for '${migration.name}'. Target objects: [${valResult.affectedTables.join(', ')}]`,
          );
        }
      ...
      const reportPath = MigrationReportGenerator.generateReport(
        summaryItems,
        Date.now() - startTime,
        true,
      );
    ...
    if (client && client.query) {
      ...
      for (const migration of pendingMigrations) {
        ...
        if (!valResult.valid) {
          logger.error(
            `Aborting execution due to safety/validation failure in '${migration.name}'.`,
          );
          break;
        }
        ...
        const historyId = await MigrationHistoryManager.recordStart(
          client,
          migration.name,
          checksum,
        );
        ...
        try {
          ...
          logger.error(
            `Execution failed for '${migration.name}': ${err.message}. Transaction rolled back.`,
          );
