...
async function uploadDoc(token, applicationId, docTypeCode, academicYearId) {
  ...
  const blob = new Blob([`AMAT test document ${docTypeCode} ${Date.now()} ${Math.random()}`], {
...
async function ensureInterviewPanel() {
  ...
  await supabaseAdmin.rpc('exec_transaction_queries', {
    sql_queries: [
      `INSERT INTO public.admission_interview_panels (panel_name, members) VALUES ('AMAT Default Panel', ARRAY['Member A','Member B']::text[]) ON CONFLICT (panel_name) DO NOTHING`,
    ],
  });
  ...
  if (!created?.id)
    throw new Error('Failed to resolve interview panel — seed 084 or run apply-stage32-migrations');
  return created.id;
}
...
async function ensureStorageBucket() {
  ...
  if (!buckets?.some((b) => b.name === 'admission-documents')) {
    const { error } = await supabaseAdmin.storage.createBucket('admission-documents', {
      public: false,
    });
...
async function main() {
  ...
  const { data: counselorUser } = await supabaseAdmin
    .from('users')
    .select('id')
    .eq('email', 'counselor@edu.in')
    .maybeSingle();
  if (!counselorUser) fail('0', 'Counselor user missing');

  const runTag = Array.from({ length: 10 }, () =>
  ...
  const updatedAt =
    resumeRes.data?.application?.updated_at ??
    resumeRes.data?.updated_at ??
  ...
  await api(
    counselorToken,
    'PATCH',
    `/v1/admission/application/${applicationId}/profile`,
    {
      date_of_birth: dateOfBirth,
      gender: 'Male',
    },
  ...
  await api(
    counselorToken,
    'PATCH',
    `/v1/admission/application/${applicationId}/parents`,
    {
  ...
  await api(
    counselorToken,
    'PATCH',
    `/v1/admission/application/${applicationId}/declaration`,
    {
  ...
  const submitRes = await api(
    adminToken,
    'POST',
    `/v1/admission/application/${applicationId}/submit`,
    {
  ...
  for (const docId of uploadedIds.filter(Boolean)) {
    const ver = await api(
      officerToken,
      'POST',
      `/v1/admission/application/documents/${docId}/verify`,
      { remarks: 'AMAT verified' },
    );
  ...
  const scheduleExamRes = await api(
    examCellToken,
    'POST',
    '/v1/admission/evaluation/exam/schedule',
    {
  ...
  let { data: feeStructure } = await supabaseAdmin
  ...
  const outstandingTotal = (feeAssignments ?? []).reduce(
    (sum, row) =>
      ...
      Math.max(
        0,
        Number(row.amount ?? 0) - Number(row.waived_amount ?? 0) - Number(row.paid_amount ?? 0),
      ),
    0,
  );
  ...
  const reviewRes = await api(
    officerToken,
    'POST',
    `/v1/admission/application/${applicationId}/review`,
    { remark: 'Committee review AMAT' },
  );
  ...
  const approveRes = await api(
    principalToken,
    'POST',
    `/v1/admission/application/${applicationId}/approve`,
    { remark: 'Principal approval AMAT' },
  );
  ...
  const confirmRes = await api(officerToken, 'POST', '/v1/admission/enrollment/confirm', {
    application_id: applicationId,
  });
  if (!confirmRes.ok) fail('11', 'Confirm admission failed', confirmRes);

  const enrollRes = await api(officerToken, 'POST', '/v1/admission/enrollment/enroll', {
    application_id: applicationId,
  });
  ...
  const { count: dupStudents } = await supabaseAdmin
    .from('students')
    .select('*', { count: 'exact', head: true })
    .eq('admission_no', confirmation.admission_number);
  if ((dupStudents ?? 0) !== 1) fail('12', 'Duplicate student records detected', { dupStudents });

  const { data: student } = await supabaseAdmin
    .from('students')
    .select('id, first_name, status')
    .eq('id', confirmation.student_id)
    .maybeSingle();
  ...
  const progressRes = await api(
    officerToken,
    'GET',
    `/v1/admission/application/${applicationId}/progress`,
  );
  pass('13', 'Progress API', {
    progressPercent: progressRes.data?.progressPercent ?? progressRes.data?.data?.progressPercent,
  });
