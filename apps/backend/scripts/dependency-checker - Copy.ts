  ...
  public static verifySafety(
    ...
    for (const tableName of DependencyChecker.PROTECTED_TABLES) {
      ...
      const dropRegex = new RegExp(
        `drop\\s+table\\s+(if\\s+exists\\s+)?(public\\.)?${tableName}\\b`,
        'i',
      );
      const alterDropRegex = new RegExp(
        `alter\\s+table\\s+(public\\.)?${tableName}\\s+drop\\b`,
        'i',
      );

      if (dropRegex.test(normalizedSql) || alterDropRegex.test(normalizedSql)) {
        violations.push(
          `Migration '${fileName}' attempts to DROP protected object '${tableName}'.`,
        );
      }
    }
