...
async function main() {
  ...
  const uniqueSuffix = TIMESTAMP.slice(-6)
    .split('')
    .map((d) => 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'[parseInt(d, 10) % 26])
    .join('');
  ...
  const enquiryAfterAssign = await api(
    receptionistToken,
    'GET',
    `/v1/admission/crm/enquiries/${enquiryId}`,
  );
  ...
  if (!assignedBy)
    fail('2', 'assigned_by not populated from assignment audit', enquiryAfterAssign.data);
  ...
  if (!leadRecord)
    fail('3', 'Assigned lead not visible to counselor', { enquiryId, leadCount: leads.length });
  ...
  if (convertRes.status !== 200 && convertRes.status !== 201)
    fail('4', 'Convert failed', convertRes);
  const { lead_id, application_id } = convertRes.data || {};
  if (!lead_id || !application_id)
    fail('4', 'Response missing lead_id or application_id', convertRes.data);
  pass('4', 'Convert succeeded', convertRes.data);

  // Idempotent re-convert must return same application
  const convert2 = await api(
    counselorToken,
    'POST',
    `/v1/admission/crm/enquiries/${enquiryId}/convert`,
    {},
  ...
  const timelineRes = await api(
    counselorToken,
    'GET',
    `/v1/admission/application/${application_id}/timeline`,
  );
  ...
  const { data: dbEnquiry, error: dbEnquiryErr } = await supabaseAdmin
    ...
    .eq('id', enquiryId)
    .maybeSingle();
  if (dbEnquiryErr || !dbEnquiry)
    fail('7', 'Enquiry row not found in DB', { enquiryId, dbEnquiryErr });
  ...
  if ((dbLeads || []).length !== 1)
    fail('7', `Expected 1 lead, got ${(dbLeads || []).length}`, dbLeads);
  if ((dbApps || []).length !== 1)
    fail('7', `Expected 1 application, got ${(dbApps || []).length}`, dbApps);
  const appRow = dbApps[0];
  if (appRow.school_id !== school.id) fail('7', 'Application school_id mismatch', appRow);
  if (appRow.academic_year_id !== academicYearId)
    fail('7', 'Application academic_year_id mismatch', appRow);
