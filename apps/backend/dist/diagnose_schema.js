...
async function diagnose() {
    ...
    const { data: examsData, error: examsError } = await supabase_1.supabase.from('exams').select('*').limit(1);
    if (examsError) {
        ...
        for (const col of columns) {
            const { error } = await supabase_1.supabase.from('exams').select(col).limit(1);
            if (error) {
    ...
    const { data: seatData, error: seatError } = await supabase_1.supabase.from('exam_seating_allocations').select('*').limit(1);
    if (seatError) {
        ...
        for (const col of columns) {
            const { error } = await supabase_1.supabase.from('exam_seating_allocations').select(col).limit(1);
            if (error) {
                console.log(`Column [${col}] is MISSING: ${error.message}`);
            }
            else {
    ...
    const { data: hallData, error: hallError } = await supabase_1.supabase.from('exam_halls').select('*').limit(1);
    if (hallError) {
    ...
    const { error: vError } = await supabase_1.supabase.from('exam_seating_versions').select('id').limit(1);
    if (vError) {
