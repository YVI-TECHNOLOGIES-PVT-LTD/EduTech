...
exports.StudentMasterProvisioner = void 0;
const supabase_1 = require("../../../../../config/supabase");
class StudentMasterProvisioner {
    async provision(applicationId, admissionNumber, studentProfile) {
    ...
    async rollback(studentId) {
        const { error } = await supabase_1.supabase.from('students').delete().eq('id', studentId);
        if (error && !error.message.includes('does not exist')) {
            throw error;
        }
    }
}
exports.StudentMasterProvisioner = StudentMasterProvisioner;
