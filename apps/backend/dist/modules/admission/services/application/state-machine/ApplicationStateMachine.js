...
class ApplicationStateMachine {
    ...
    async validateTransition(fromStatus, toStatus, role) {
        ...
        const isAllowed = await this.appRepo.getWorkflowRule(fromStatus.toLowerCase(), toStatus.toLowerCase(), role) ||
        ...
        if (!isAllowed) {
            ...
            if (!allowed) {
                throw new BusinessRuleError_1.BusinessRuleError(`Invalid workflow transition from [${fromStatus}] to [${toStatus}] for role [${role}].`);
            }
            ...
            if (normalizedRole !== 'ADMIN') {
                if (['DRAFT', 'DOCS_PENDING'].includes(uFrom) && uTo === 'SUBMITTED') {
                    if (normalizedRole !== 'PARENT' && normalizedRole !== 'COUNSELOR') {
                        throw new BusinessRuleError_1.BusinessRuleError(`Forbidden: Only Parent or Counselor can submit applications.`);
                    }
                }
                if (uTo === 'DOCUMENT_VERIFIED' || uTo === 'DOCS_PENDING') {
                    if (normalizedRole !== 'ADMISSION_OFFICER') {
                        throw new BusinessRuleError_1.BusinessRuleError(`Forbidden: Only Admission Officer can verify or request documents.`);
                    }
                }
                if (['EXAM', 'INTERVIEW', 'MERIT'].includes(uTo)) {
                    if (normalizedRole !== 'EXAM_CELL' && normalizedRole !== 'EXAM_CELL_ADMIN') {
                        throw new BusinessRuleError_1.BusinessRuleError(`Forbidden: Only Exam Cell can manage exam, interview or merit list states.`);
                    }
                }
                if (uTo === 'OFFERED') {
                    if (normalizedRole !== 'PRINCIPAL' && normalizedRole !== 'HOI' && normalizedRole !== 'HEAD_OF_INSTITUTE') {
                        throw new BusinessRuleError_1.BusinessRuleError(`Forbidden: Only Principal / HOI can approve offers.`);
                    }
                }
                if (uTo === 'FEE_VERIFIED') {
                    if (normalizedRole !== 'FINANCE_OFFICER' && normalizedRole !== 'ACCOUNTANT') {
                        throw new BusinessRuleError_1.BusinessRuleError(`Forbidden: Only Finance Officer can verify payments.`);
                    }
                }
                if (uTo === 'ENROLLED') {
                    if (normalizedRole !== 'ADMISSION_OFFICER') {
                        throw new BusinessRuleError_1.BusinessRuleError(`Forbidden: Only Admission Officer can confirm final enrollment.`);
                    }
                }
            }
        }
