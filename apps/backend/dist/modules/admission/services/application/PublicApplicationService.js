...
class PublicApplicationService {
    ...
    static normalizePublicApplicationPayload(rawPayload) {
        ...
        const regToken = (rawPayload.registrationToken || rawPayload.registration_token || rawPayload.proof_token || rawPayload.proofToken || '').trim();
    ...
    static async apply(payload) {
        ...
        // 1b. Validate OTP Registration Proof Token BEFORE starting database transaction
        const registrationToken = canonical.registration_token;
        if (!registrationToken) {
            throw new ValidationError_1.ValidationError('Registration proof token (registrationToken) is required for public application submission.');
        }
        if (!canonical.contact_phone) {
            throw new ValidationError_1.ValidationError('Contact phone number is required for application submission.');
        }
        const normalizedPhone = (0, phone_utils_1.normalizePhone)(canonical.contact_phone);
        // Validates proof token against orgId + phone, and consumes it single-use
        await otp_service_1.otpService.validateRegistrationProof(registrationToken, finalOrgId, normalizedPhone);
        ...
        // 4. Run Transaction for User, Parent, Role, Lead, and Application creation
        return prismaClient_1.default.$transaction(async (tx) => {
            // a. Find or create user by email or normalized phone
            ...
            // b. Find, create, or link Parent record in DB
            let parent = await tx.parents.findFirst({
                where: {
                    org_id: finalOrgId,
                    OR: [
            ...
            if (!parent) {
                parent = await tx.parents.create({
                    data: {
                        org_id: finalOrgId,
                        user_id: user.user_id,
                        first_name: canonical.parent_first_name || 'Parent User',
                ...
                console.log(`[PUBLIC-APPLY] parent record created: ${parent.parent_id}`);
            }
                ...
                console.log(`[PUBLIC-APPLY] parent record linked to user: ${parent.parent_id}`);
            }
            // c. Ensure PARENT role is assigned
