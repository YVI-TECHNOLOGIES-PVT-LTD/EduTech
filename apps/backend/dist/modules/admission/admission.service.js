...
class AdmissionService {
    ...
    static async resolveContext() {
        const { data: school, error: schoolError } = await supabase_1.supabase.from('schools').select('id').limit(1).single();
    ...
    static async createPublicApplication(data) {
        ...
        if (data.parent_password && data.parent_email) {
            ...
            const { data: role } = await supabase_1.supabase.from('roles').select('id').eq('name', 'PARENT').single();
            if (role) {
    ...
    static async getApplications(filters, page = 1, limit = 10, search) {
        let query = supabase_1.supabase.from('admissions').select('*, academic_years(year_label)', { count: 'exact' });
        ...
        if (search) {
            query = (0, queryHelpers_1.applySearch)(query, search, ['student_name', 'parent_name', 'parent_phone', 'id']);
        }
    ...
    static async getStats(schoolId) {
        let query = supabase_1.supabase.from('admissions').select('id, status, created_at, updated_at');
        if (schoolId)
    ...
    static async enrolApplicant(id, adminId) {
        // 1. Fetch admission
        const { data: app, error: fetchError } = await supabase_1.supabase
            .from('admissions')
            .select('*')
            .eq('id', id)
            .single();
        ...
        if (existingStudent)
            throw new Error('Applicant is already enrolled as a student');
        // 3. Create student
        ...
        if (studentError)
            throw studentError;
        // 4. Link parent
        await supabase_1.supabase.from('student_parents').insert({
            student_id: student.id,
            parent_user_id: app.applicant_user_id,
            relation: 'guardian'
        });
        // 5. Update admission status to enrolled
        await supabase_1.supabase.from('admissions').update({ status: 'enrolled' }).eq('id', id);
        // 6. Log audit
    ...
    static async submitPayment(id, userId, paymentData) {
        const { data, error } = await supabase_1.supabase
            .from('admissions')
            .update({
    ...
    static async logAudit(admissionId, action, performedBy, remarks) {
    ...
    static async uploadDocument(admissionId, type, url) {
        const { data, error } = await supabase_1.supabase
    ...
    static async decideLogin(admissionId, adminId, status, reason) {
        // 1. Fetch admission to get the parent/student user ID
        const { data: app, error: appError } = await supabase_1.supabase
            .from('admissions')
            .select('applicant_user_id')
            .eq('id', admissionId)
            .single();
        if (appError || !app?.applicant_user_id)
            throw new Error('Admission or applicant not found');
    ...
    static async initializeBilling(admissionId, performedBy, feeStructureIds) {
        ...
        const { data: structures, error: structError } = await supabase_1.supabase
            .from('fee_structures')
            .select('*')
        ...
        const { data: allMandatory } = await supabase_1.supabase
            .from('fee_structures')
            .select('*')
        ...
        if (allMandatory) {
            ...
            const applicableMandatory = allMandatory.filter(mf => !mf.applicable_classes ||
        ...
        await supabase_1.supabase.from('admission_fees').delete().eq('admission_id', admissionId);
        ...
        if (updateError)
            throw updateError;
        // 7. Audit Logging
        await this.logAudit(admissionId, 'BILLING_INITIALIZED', performedBy, `Initialized with ${structures.length} components. Total Amount: ₹${totalAmount.toLocaleString()}`);
        return {
            success: true,
