...
class EvaluationRepository {
    async getEvaluationSummary(applicationId) {
        ...
        const { data: candidate, error: candidateErr } = await supabase_1.supabase
        ...
        let examResult = null;
        if (candidate) {
            const { data: results, error: resultsErr } = await supabase_1.supabase
        ...
        const { data: interview, error: interviewErr } = await supabase_1.supabase
        ...
        if (interview) {
            const { data: scores, error: scoresErr } = await supabase_1.supabase
        ...
        const { data: merit, error: meritErr } = await supabase_1.supabase
        ...
        if (meritErr)
            throw meritErr;
        // Query offers status
        const { data: offer, error: offerErr } = await supabase_1.supabase
