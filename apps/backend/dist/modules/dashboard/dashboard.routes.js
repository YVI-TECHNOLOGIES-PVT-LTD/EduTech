...
// ADMIN OVERVIEW
// ======================================
exports.dashboardRouter.get('/admin/overview', (0, rbac_middleware_1.checkPermission)(permissions_1.PERMISSIONS.DASHBOARD_VIEW_ADMIN), async (req, res) => {
    const schoolId = req.context.user.school_id;
    try {
        const today = new Date().toISOString().split('T')[0];
        const [{ count: students }, { count: classes }, { count: exams }, { count: pendingAdmissions }, { count: totalApplications }, { data: allPayments }, { count: sessionsToday }] = await Promise.all([
            supabase_1.supabase.from('students').select('*', { count: 'exact', head: true }).eq('school_id', schoolId).eq('status', 'active'),
            supabase_1.supabase.from('classes').select('*', { count: 'exact', head: true }).eq('school_id', schoolId),
            supabase_1.supabase.from('exams').select('*', { count: 'exact', head: true }).eq('school_id', schoolId),
            supabase_1.supabase.from('admission_applications').select('*', { count: 'exact', head: true }).eq('school_id', schoolId).in('status', ['SUBMITTED', 'UNDER_REVIEW', 'DOCS_PENDING']),
            supabase_1.supabase.from('admission_applications').select('*', { count: 'exact', head: true }).eq('school_id', schoolId).eq('is_current', true).is('deleted_at', null),
            // PHASE-A: Real Fee Collection (Lifetime) - Note: In prod, replace with RPC for scale
            supabase_1.supabase.from('payments').select('amount_paid'),
            // PHASE-A: Real Attendance Activity (Today)
            supabase_1.supabase.from('attendance_sessions').select('*', { count: 'exact', head: true }).eq('date', today)
        ]);
        // Compute Totals
...
// FACULTY OVERVIEW
// ======================================
exports.dashboardRouter.get('/faculty/overview', (0, rbac_middleware_1.checkPermission)(permissions_1.PERMISSIONS.DASHBOARD_VIEW_FACULTY), async (req, res) => {
    const userId = req.context.user.id;
    try {
        // 1. Sections Handled
        const { count: sections } = await supabase_1.supabase
        ...
        const { data: todayClasses } = await supabase_1.supabase
...
// LIVE TIMELINE (Personalized)
// ======================================
exports.dashboardRouter.get('/timeline', async (req, res) => {
    ...
    try {
        ...
        if (isAdmin || isParent || isStudent) {
            let query = supabase_1.supabase.from('admission_audit_logs').select(`
            ...
            const filteredAdmissions = admissionsEvents?.filter(e => {
                ...
                if (isParent || isStudent)
                    return admission.applicant_user_id === userId;
                return false;
            }).map(e => ({
                id: e.id,
                type: 'ADMISSION',
                title: e.admission?.student_name,
        ...
        if (isAdmin || isFaculty || isParent || isStudent) {
            let query = supabase_1.supabase.from('attendance_sessions').select(`
            ...
            const filteredAtt = attSessions?.map(s => ({
                id: s.id,
                type: 'ATTENDANCE',
                title: `${s.section?.class?.name} - ${s.section?.name}`,
        ...
        // 3. ASSIGNMENTS
        if (isAdmin || isFaculty || isParent || isStudent) {
            const { data: assignments } = await supabase_1.supabase.from('assignments').select(`
            ...
            const filteredAssignments = assignments?.filter(a => {
                if (isAdmin)
                    return true;
                if (isFaculty)
                    return a.teacher_user_id === userId;
                return true; // Parent/Student see all for now (demo mode style)
            }).map(a => ({
                id: a.id,
                type: 'ASSIGNMENT',
                title: a.title,
                description: `New assignment in ${a.section?.class?.name}`,
                time: a.created_at,
                icon: 'BookOpen',
                color: 'indigo'
            })) || [];
        ...
        if (isAdmin || isParent || isStudent) {
            ...
            const { data: payments } = await supabase_1.supabase.from('payments').select(`
            ...
            const filteredPayments = payments?.filter(p => {
                ...
                description: `Received for ${p.student?.full_name}`,
                time: p.created_at,
                icon: 'CreditCard',
                color: 'emerald'
            })) || [];
...
// PARENT OVERVIEW
// ======================================
exports.dashboardRouter.get('/parent/overview', (0, rbac_middleware_1.checkPermission)(permissions_1.PERMISSIONS.DASHBOARD_VIEW_PARENT), async (req, res) => {
    const userId = req.context.user.id;
    // 1. Fetch children summary (Enrolled Students)
    const { data: children } = await supabase_1.supabase
    ...
    const { data: crmApps } = await supabase_1.supabase
    ...
    const admissions = (crmApps ?? []).map((app) => ({
        id: app.id,
