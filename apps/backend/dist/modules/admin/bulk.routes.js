...
const production_middleware_1 = require("../../middleware/production.middleware");
exports.bulkRouter = (0, express_1.Router)();
const upload = (0, multer_1.default)({ storage: multer_1.default.memoryStorage() });
// Helper to prevent modifications to CLOSED years
const checkYearNotClosed = async (yearId) => {
...
exports.bulkRouter.post('/student-section/assign', upload.single('file'), (0, rbac_middleware_1.checkPermission)(permissions_1.PERMISSIONS.STUDENT_ASSIGN_SECTION), async (req, res) => {
    if (!req.file)
        ...
        for (const row of rows) {
            ...
            try {
                ...
                const { data: student } = await supabase_1.supabase
                    ...
                    .single();
                if (!student)
                    throw new Error(`Student ${student_code} not found`);
                ...
                const { data: section } = await supabase_1.supabase
                    ...
                    .single();
                if (!section)
                    throw new Error(`Section ${section_code} in Class ${class_code} not found for this year`);
                // 3. Call Atomic RPC with Capacity Guard
                const { error: upsertError } = await supabase_1.supabase.rpc('fn_assign_student_with_capacity_guard', {
                ...
                await academic_service_1.AcademicAssignmentService.syncStudentWithSectionFaculty(student.id, section.id, academicYearId);
                // 4. Log to Timeline
...
exports.bulkRouter.post('/students/promote', upload.single('file'), (0, rbac_middleware_1.checkPermission)(permissions_1.PERMISSIONS.STUDENT_ASSIGN_SECTION), async (req, res) => {
    if (!req.file)
        return res.status(400).json({ error: "File is required" });
    ...
    if (!from_academic_year_id)
        return res.status(400).json({ error: "from_academic_year_id is required" });
    try {
        const rows = parseFile(req.file);
        const report = {
            total: rows.length,
        ...
        const { data: activeExams } = await supabase_1.supabase
            .from('exams')
            .select('id, name')
        ...
        for (const row of rows) {
            ...
            try {
                // Resolve Data
                const { data: student } = await supabase_1.supabase.from('students').select('id, admission_id').eq('student_code', student_code).eq('school_id', schoolId).single();
                if (!student)
                    throw new Error("Student not found");
                const { data: targetYear } = await supabase_1.supabase.from('academic_years').select('id, status').eq('year_label', target_academic_year).eq('school_id', schoolId).single();
                if (!targetYear)
                    throw new Error(`Target year ${target_academic_year} not found`);
                if (targetYear.id === from_academic_year_id)
                    throw new Error("Target year cannot be same as source");
                if (targetYear.status === 'CLOSED')
                    throw new Error("Target year is CLOSED");
                const { data: section } = await supabase_1.supabase
                    .from('sections')
                    ...
                    .single();
                if (!section)
                    throw new Error(`Target section ${target_section} in Class ${target_class} not found for ${target_academic_year}`);
                if (!is_dry_run) {
                    const { error: promoError } = await supabase_1.supabase
                    ...
                    if (promoError)
                        throw promoError;
                    await academic_service_1.AcademicAssignmentService.syncStudentWithSectionFaculty(student.id, section.id, targetYear.id);
                    if (student.admission_id) {
                        await supabase_1.supabase.from('admission_audit_logs').insert({
                            admission_id: student.admission_id,
...
exports.bulkRouter.post('/attendance/seed', production_middleware_1.blockInProduction, upload.single('file'), (0, rbac_middleware_1.checkPermission)(permissions_1.PERMISSIONS.DASHBOARD_VIEW_ADMIN), async (req, res) => {
    if (!req.file)
        return res.status(400).json({ error: "File is required" });
    ...
    try {
        ...
        const rows = parseFile(req.file);
        const report = { total: rows.length, success: 0, failure: 0, details: [] };
        ...
        for (const row of rows) {
            ...
            try {
                ...
                const { data: student } = await supabase_1.supabase
                    ...
                    .single();
                if (!student)
                    throw new Error(`Student ${student_code} not found or not assigned to a section in this year`);
                const sectionId = student.student_sections[0].section_id;
                ...
                if (!sessionId) {
                    ...
                    if (session) {
                        ...
                        const { data: newSess, error: insErr } = await supabase_1.supabase
                            .from('attendance_sessions')
                            .insert({
                            school_id: schoolId,
                            academic_year_id,
                ...
                // 3. Upsert Record
                const { error: recErr } = await supabase_1.supabase
...
exports.bulkRouter.post('/fees/seed', production_middleware_1.blockInProduction, upload.single('file'), (0, rbac_middleware_1.checkPermission)(permissions_1.PERMISSIONS.DASHBOARD_VIEW_ADMIN), async (req, res) => {
    if (!req.file)
        return res.status(400).json({ error: "File is required" });
    const schoolId = req.context.user.school_id;
    try {
        ...
        for (const row of rows) {
            ...
            try {
                ...
                const { data: student } = await supabase_1.supabase.from('students').select('id').eq('student_code', student_code).eq('school_id', schoolId).single();
                if (!student)
                    throw new Error("Student not found");
                const term = detectTerm(fee_type);
                // 1. Resolve or Create Fee Structure
                let { data: structure } = await supabase_1.supabase
                    .from('fee_structures')
                    .select('id')
                    .eq('school_id', schoolId)
                    ...
                    .maybeSingle();
                if (!structure) {
                    const { data: newStructure, error: structError } = await supabase_1.supabase
                        .from('fee_structures')
                        .insert({
                        school_id: schoolId,
                ...
                const { data: sFee, error: sFeeErr } = await supabase_1.supabase.from('student_fees').upsert({
                    student_id: student.id,
                ...
                if (Number(paid_amount) > 0) {
                    const { error: payError } = await supabase_1.supabase.rpc('fn_record_payment_with_allocation', {
                        p_student_id: student.id,
 ...
 */
exports.bulkRouter.post('/class/auto-distribute', (0, rbac_middleware_1.checkPermission)(permissions_1.PERMISSIONS.STUDENT_ASSIGN_SECTION), async (req, res) => {
    const { classId, academicYearId } = req.body;
    const schoolId = req.context.user.school_id;
    const userId = req.context.user.id;
    if (!classId || !academicYearId)
        return res.status(400).json({ error: "classId and academicYearId required" });
    try {
        await checkYearNotClosed(academicYearId);
        // Call Atomic RPC
        const { data, error } = await supabase_1.supabase.rpc('fn_auto_distribute_class', {
