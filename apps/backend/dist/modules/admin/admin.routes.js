...
exports.adminRouter.post('/student-section/assign', (0, rbac_middleware_1.checkPermission)(permissions_1.PERMISSIONS.STUDENT_ASSIGN_SECTION), async (req, res) => {
    ...
    try {
        ...
        const { data: activeYear, error: yearError } = await supabase_1.supabase
            .from('academic_years')
            .select('id, status')
        ...
        const sectionYearId = section.class.academic_year_id;
        ...
        await academic_service_1.AcademicAssignmentService.syncStudentWithSectionFaculty(student_id, section_id, activeYear.id);
        // 6. Log to Timeline
...
exports.adminRouter.post('/students/promote', (0, rbac_middleware_1.checkPermission)(permissions_1.PERMISSIONS.STUDENT_ASSIGN_SECTION), async (req, res) => {
    ...
    try {
        ...
        const { data: activeExams, error: examError } = await supabase_1.supabase
            .from('exams')
            .select('id, name')
        ...
        for (const studentId of student_ids) {
            try {
                ...
                if (promoError)
                    throw promoError;
                // Sync Faculty
                await academic_service_1.AcademicAssignmentService.syncStudentWithSectionFaculty(studentId, target_section_id, to_academic_year_id);
                // Log to Timeline
                const { data: student } = await supabase_1.supabase.from('students').select('admission_id').eq('id', studentId).single();
                if (student?.admission_id) {
                    await supabase_1.supabase.from('admission_audit_logs').insert({
                        admission_id: student.admission_id,
...
exports.adminRouter.post('/academic-years/activate', (0, rbac_middleware_1.checkPermission)(permissions_1.PERMISSIONS.CLASS_CREATE), // Reusing high-level academic permission
async (req, res) => {
    ...
    try {
        ...
        const { data: targetYear } = await supabase_1.supabase
            .from('academic_years')
            .select('status')
        ...
        const { data: currentActive } = await supabase_1.supabase
            .from('academic_years')
        ...
        // 4. Activate new year
        const { error: activeError } = await supabase_1.supabase
            .from('academic_years')
...
exports.adminRouter.post('/academic-years/close', (0, rbac_middleware_1.checkPermission)(permissions_1.PERMISSIONS.CLASS_CREATE), async (req, res) => {
    const { academic_year_id } = req.body;
    const schoolId = req.context.user.school_id;
    try {
