...
exports.DriverVehicleMappingStrategy = void 0;
const supabase_1 = require("../../../config/supabase");
const zod_1 = require("zod");
// Schema: Driver Identifier + Vehicle Identifier
const MappingRowSchema = zod_1.z.object({
    driver_identifier: zod_1.z.string().min(1, "Driver Email or Phone is required").transform(val => val.trim().toLowerCase()),
    vehicle_no: zod_1.z.string().min(1, "Vehicle Number is required").transform(val => val.trim().toUpperCase())
});
class DriverVehicleMappingStrategy {
    async validate(rows, schoolId) {
        ...
        const driverMap = new Map(); // identifier (email/phone) -> driver_id
        const vehicleMap = new Map(); // vehicle_no -> vehicle_id
        if (identifiers.length > 0) {
            ...
            if (userData) {
                userData.forEach((d) => {
                    ...
                    if (d.phone)
                        driverMap.set(d.phone, d.id);
                });
            }
        }
        if (vehicles.length > 0) {
            const { data: vData } = await supabase_1.supabase
                .from('transport_vehicles')
                .select('id, vehicle_no')
                .eq('school_id', schoolId)
                .in('vehicle_no', vehicles);
            if (vData) {
                vData.forEach((v) => vehicleMap.set(v.vehicle_no, v.id));
            }
        }
        const seenInBatch = new Set(); // composite key
        // 2. Validate
        for (const row of normalizedRows) {
            ...
            if (!parsed.success) {
                parsed.error.errors.forEach(err => errors.push({
                    row: rowNum, column: err.path.join('.'), message: err.message, value: rows[rowNum - 1][err.path[0]]
                }));
            }
            else {
...
exports.DriverVehicleMappingStrategy = DriverVehicleMappingStrategy;
