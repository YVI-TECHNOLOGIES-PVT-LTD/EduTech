...
const FacultyRowSchema = zod_1.z.object({
    ...
    department_id: zod_1.z.string().uuid("Invalid Department UUID"),
    designation: zod_1.z.string().min(1, "Designation is required")
});
class FacultyImportStrategy {
    constructor() {
    ...
    async execute(validRows, context) {
        ...
        if (!this.facultyRoleId) {
            const { data } = await supabase_1.supabase.from('roles').select('id').eq('name', 'FACULTY').single();
            if (!data)
        ...
        for (const row of validRows) {
            ...
            try {
                ...
                if (roleError)
                    throw new Error(`Role failed: ${roleError.message}`);
                ...
                // COMPENSATION
                if (authUserId) {
                    await supabase_1.supabase.auth.admin.deleteUser(authUserId).catch(console.error);
                }
                result.failedCount++;
