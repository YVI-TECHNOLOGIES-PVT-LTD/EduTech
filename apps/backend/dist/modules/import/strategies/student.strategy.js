    ...
    async execute(validRows, context) {
        ...
        for (let i = 0; i < validRows.length; i++) {
            ...
            try {
                ...
                const { error: stuError } = await supabase_1.supabase
                    .from('students')
                    .insert({
                    school_id: context.schoolId,
                ...
                if (row.section_name) {
                    ...
                    if (section) {
                        const { error: ssError } = await supabase_1.supabase.rpc('fn_assign_student_with_capacity_guard', {
                            p_student_id: (await supabase_1.supabase.from('students').select('id').eq('student_code', row.student_code).single()).data?.id,
