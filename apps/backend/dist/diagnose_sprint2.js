...
async function diagnose() {
    ...
    for (const table of tables) {
        const { error } = await supabase_1.supabase.from(table).select('*').limit(1);
        if (error) {
    ...
    for (const col of enqColumns) {
        const { error } = await supabase_1.supabase.from('admission_enquiries').select(col).limit(1);
        if (error) {
    ...
    for (const col of visColumns) {
        const { error } = await supabase_1.supabase.from('admission_visitors').select(col).limit(1);
        if (error) {
