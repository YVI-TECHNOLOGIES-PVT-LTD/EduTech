...
class WorkflowScheduler {
    ...
    static async checkSLAs() {
        ...
        try {
            ...
            const { data: pendingRequests } = await supabase_1.supabase
            ...
            for (const req of pendingRequests) {
                ...
                const step = ver.nodes.find((n) => n.id === req.node_id);
                if (!step)
                    continue;
                ...
                if (hrsDiff >= slaWarningHrs && !req.metadata?.warning_sent) {
                    ...
                    if (req.assigned_user_id) {
                        await NotificationService_1.NotificationService.send(req.assigned_user_id, "SLA WARNING: Task Pending Review", `Your approval task "${step.data?.label || 'Step'}" is pending more than ${slaWarningHrs} hours.`);
                    }
                }
