...
class WorkflowService {
     ...
     */
    static async startWorkflow(workflowCode, entityType, entityId, schoolId, performedBy) {
        try {
            console.log(`[WorkflowService] Starting workflow "${workflowCode}" for ${entityType} ID: ${entityId}`);
            // 1. Fetch active workflow & version
            const { data: wf } = await supabase_1.supabase
                .from('workflows')
                .select('id')
            ...
            const { data: ver } = await supabase_1.supabase
                ...
                .single();
            if (!ver)
                throw new Error(`No published version found for workflow "${workflowCode}"`);
            // 2. Fetch context variables
            const { data: vars } = await supabase_1.supabase
                .from('workflow_variables')
                .select('*')
                .eq('workflow_version_id', ver.id);
            // Fetch target entity to build context
            const { data: entity } = await supabase_1.supabase.from(entityType + 's').select('*').eq('id', entityId).single();
            if (!entity)
                throw new Error(`Target ${entityType} record not found`);
            ...
            // Find Start node
            const startNode = ver.nodes.find((n) => n.type === 'start' || n.type === 'Start');
            if (!startNode)
                throw new Error("Workflow schema has no Start node");
            // 3. Create workflow run
            const { data: run, error: createError } = await supabase_1.supabase
                .from('workflow_runs')
                .insert({
                version_id: ver.id,
     ...
     */
    static async executeNode(runId, nodeId, entityContext, performedBy) {
        try {
            // Get run state
            const { data: run } = await supabase_1.supabase.from('workflow_runs').select('*, workflow_versions(*)').eq('id', runId).single();
            ...
            if (!node)
                throw new Error(`Node ID "${nodeId}" not found in layout schema`);
            console.log(`[WorkflowService] Executing node "${nodeId}" of type "${node.type}"`);
            // Update run current node pointer
            await supabase_1.supabase.from('workflow_runs').update({ current_node_id: nodeId }).eq('id', runId);
            switch (node.type?.toLowerCase()) {
                ...
                case 'approval': {
                    ...
                    const { data: appReq } = await supabase_1.supabase
                        ...
                        .select()
                        .single();
                    const { data: task } = await supabase_1.supabase
                        .from('tasks')
                        .insert({
                        ...
                        .select()
                        .single();
                    await supabase_1.supabase.from('workflow_runs').update({ status: 'waiting' }).eq('id', runId);
                ...
                case 'condition': {
                    ...
                    const { data: condRecord } = await supabase_1.supabase
                    ...
                    const evaluationOutcome = rule_evaluator_1.RuleEvaluator.evaluate(conditionConfig, entityContext);
                ...
                case 'action': {
                    ...
                    const { data: actionsList } = await supabase_1.supabase
                    ...
                    if (actionsList && actionsList.length > 0) {
                        for (const action of actionsList) {
                            try {
                                ...
                                // Add details to DLQ
                                await supabase_1.supabase.from('workflow_dlq').insert({
                                    workflow_run_id: runId,
                                    node_id: nodeId,
                ...
                case 'notification': {
                    ...
                    const { data: notification } = await supabase_1.supabase
                    ...
                    if (notification) {
                        ...
                        if (entityContext.applicant_user_id) {
                            await NotificationService_1.NotificationService.send(entityContext.applicant_user_id, "Syllabus Alert", message, { run_id: runId });
                        }
                    }
                    const nextNodeId = this.getNextNode(version.connections, nodeId);
                    if (nextNodeId) {
     ...
     */
    static async submitDecision(runId, nodeId, userId, decision, remarks) {
        try {
            console.log(`[WorkflowService] Decision on run ${runId} node ${nodeId} by user ${userId}: ${decision}`);
            // Update pending approval request
            const { data: req, error: reqError } = await supabase_1.supabase
                .from('approval_requests')
                .update({
                ...
                .single();
            if (reqError)
                throw new Error("No pending approval request matches parameters");
            ...
            const { data: entity } = await supabase_1.supabase.from(entityType + 's').select('*').eq('id', run.entity_id).single();
            if (decision === 'approved') {
                // Return run to active tracking and proceed
                await supabase_1.supabase.from('workflow_runs').update({ status: 'running' }).eq('id', runId);
                ...
                await supabase_1.supabase.from('workflow_runs').update({ status: 'cancelled' }).eq('id', runId);
     ...
     */
    static async runActionBlock(runId, action, entity) {
        console.log(`[WorkflowService] Dispatching Action: ${action.action_type}`);
        const config = action.action_config || {};
        switch (action.action_type) {
            case 'create_task': {
                await supabase_1.supabase.from('tasks').insert({
    ...
    static getNextNode(connections, sourceNodeId) {
        const edge = connections.find((c) => c.source === sourceNodeId);
        return edge ? edge.target : null;
    }
    static getNextNodeForCondition(connections, sourceNodeId, outcome) {
        const handleStr = outcome ? 'true' : 'false';
        const edge = connections.find((c) => c.source === sourceNodeId && (c.sourceHandle === handleStr || c.label === handleStr));
        return edge ? edge.target : this.getNextNode(connections, sourceNodeId); // Fallback
    }
    static async completeRun(runId, performedBy) {
        await supabase_1.supabase.from('workflow_runs').update({ status: 'completed' }).eq('id', runId);
    ...
    static async logAudit(runId, nodeId, action, userId, remarks) {
        await supabase_1.supabase.from('workflow_logs').insert({
            workflow_run_id: runId,
            node_id: nodeId,
            action_taken: action,
            performed_by: userId,
    ...
    static interpolateTemplate(template, obj) {
        return template.replace(/\{\{([^}]+)\}\}/g, (_, path) => {
            const val = rule_evaluator_1.RuleEvaluator.resolvePath(obj, path.trim());
     ...
     */
    static async cloneWorkflow(workflowId, newName, newCode) {
        const { data: wf } = await supabase_1.supabase.from('workflows').select('*').eq('id', workflowId).single();
        if (!wf)
            throw new Error("Source workflow not found");
        const { data: newWf } = await supabase_1.supabase
            .from('workflows')
            .insert({ name: newName, code: newCode, description: wf.description, is_template: false })
            .select()
            .single();
        const { data: versions } = await supabase_1.supabase.from('workflow_versions').select('*').eq('workflow_id', workflowId);
        if (versions) {
            for (const ver of versions) {
                const { data: newVer } = await supabase_1.supabase
                    .from('workflow_versions')
                    .insert({
                    ...
                    .single();
                // Duplicate steps
                const { data: steps } = await supabase_1.supabase.from('workflow_steps').select('*').eq('version_id', ver.id);
                if (steps && steps.length > 0) {
                    await supabase_1.supabase.from('workflow_steps').insert(steps.map(s => ({
                ...
                const { data: conds } = await supabase_1.supabase.from('workflow_conditions').select('*').eq('version_id', ver.id);
                if (conds && conds.length > 0) {
                    await supabase_1.supabase.from('workflow_conditions').insert(conds.map(c => ({
                        version_id: newVer.id,
                        node_id: c.node_id,
                ...
                // Duplicate variables
                const { data: vars } = await supabase_1.supabase.from('workflow_variables').select('*').eq('workflow_version_id', ver.id);
                if (vars && vars.length > 0) {
                    await supabase_1.supabase.from('workflow_variables').insert(vars.map(v => ({
