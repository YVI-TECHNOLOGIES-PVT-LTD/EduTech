...
export const DashboardLayout = () => {
  ...
  // Deterministic CapabilityEngine context
  ...
  // Dynamic CapabilityEngine permission-driven menu filtration
  const filteredMenuGroups = MENU_REGISTRY.map((group) => {
    if (
      !CapabilityEngine.canAccessRoute(
        group.permission || group.permissions,
        group.featurePackage,
        capabilityContext,
      )
    ) {
      return { ...group, items: [] };
  ...
  const getMobileBottomNav = () => {
    ...
    const items = [{ label: 'Dashboard', icon: LayoutDashboard, path: landingRoute }];
  ...
  const mobileBottomNavItems = getMobileBottomNav().filter(
  ...
  const SidebarItem = ({ item }: { item: any }) => {
    ...
    const isActive =
      location.pathname === pathBase && (pathHash ? location.hash === pathHash : !location.hash);
    ...
    return (
      ...
      >
        ...
        <item.icon
          className={`w-5 h-5 shrink-0 transition-transform group-hover:scale-110 ${isActive ? 'text-primary-foreground' : 'text-muted-foreground/80 group-hover:text-primary'}`}
        />
          ...
          <div className="p-6 flex flex-col gap-2 shrink-0">
            ...
            {hasPermission('exam.dashboard.view') && isSidebarOpen && (
              <div className="mt-2 px-3 py-2 bg-gradient-to-r from-purple-500/10 to-indigo-500/10 border border-purple-500/20 rounded-xl">
                <h4 className="text-[10px] font-black text-purple-900 dark:text-purple-300 uppercase tracking-wide">
                  Examination Cell
                </h4>
                <p className="text-[8px] text-purple-600 dark:text-purple-400 font-bold">
                ...
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      <span className="font-black text-base text-gray-900 dark:text-white leading-tight">
                        EduTrack
                      </span>
                  ...
                  <button
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="p-2 bg-gray-50 dark:bg-muted/10 border border-border rounded-xl"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
            ...
            <div className="h-20 px-4 sm:px-8 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 sm:gap-4">
                ...
                >
                  ...
                  <span className="text-xs text-muted-foreground/60 flex-1 text-left font-semibold">
                    Global search...
                  </span>
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="w-10 h-10 rounded-xl text-muted-foreground hover:bg-muted/50 border border-border/40"
                    >
                      {theme === 'dark' ? (
                  ...
                  <DropdownMenuContent align="end" className="rounded-2xl w-40">
                    ...
                    <DropdownMenuItem
                      ...
                      className="text-xs font-semibold py-2 rounded-lg gap-2 cursor-pointer"
                    >
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="rounded-2xl w-56 p-3">
                    <DropdownMenuLabel className="text-[10px] font-black uppercase text-gray-400 mb-2">
                      Accent Presets
                    </DropdownMenuLabel>
                    <div className="grid grid-cols-5 gap-1.5 mb-3">
                      {(['blue', 'purple', 'emerald', 'slate', 'corporate'] as const).map(
                        (preset) => {
                          ...
                          return (
                            ...
                            >
                              {colorPreset === preset && (
                                <CheckSquare className="w-3.5 h-3.5 text-white mix-blend-difference" />
                              )}
                            </button>
                          );
                        },
                    ...
                    <DropdownMenuSeparator />
                    <DropdownMenuLabel className="text-[10px] font-black uppercase text-gray-400 mt-2">
                      Layout Density
                    </DropdownMenuLabel>
                    ...
                    <div className="flex items-center justify-between py-1.5 mt-1.5">
                      ...
                      >
                        <div
                          className={`w-4 h-4 bg-white rounded-full shadow-premium-sm transition-transform duration-200 ${reducedMotion ? 'translate-x-4' : ''}`}
                        />
                      </button>
                    </div>
                  </DropdownMenuContent>
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`w-3.5 h-3.5 text-muted-foreground/60 transition-transform ${isProfileOpen ? 'rotate-180' : ''}`}
                    />
                  </button>
                        ...
                        >
                          <div className="px-4 py-4 mb-2 bg-gray-50 dark:bg-muted/10 rounded-2xl border border-border/30">
                            ...
                            <div className="flex flex-wrap gap-1 mt-2">
                              {user?.roles.map((r) => (
                                <span
                                  key={r}
                                  className="px-2 py-0.5 bg-primary/10 text-primary border border-primary/10 rounded-full text-[8px] font-black uppercase tracking-wide"
                                >
                                  {r}
                                </span>
                              ))}
                          ...
                          <div className="space-y-0.5">
                            <Link
                              to="/app/profile"
                              onClick={() => setIsProfileOpen(false)}
                              className="flex items-center gap-3 px-3.5 py-2.5 text-xs text-muted-foreground hover:text-foreground hover:bg-muted/50 rounded-xl transition-colors font-bold"
                            >
          ...
          {mobileBottomNavItems.map((item, idx) => {
            ...
            return (
              ...
              >
                ...
                />
                <span
                  className={`text-[9px] font-black tracking-wide ${isActive ? 'opacity-100' : 'opacity-80 font-bold'}`}
                >
                  {item.label}
                </span>
                {isActive && (
