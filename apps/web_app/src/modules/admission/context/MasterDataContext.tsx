...
export function MasterDataProvider({ children }: { children: React.ReactNode }) {
  ...
  const grades = useMemo(() => {
    return (
      configQuery.data?.grades?.map((g: any) => ({
        id: g.id,
        name: g.grade_name,
  ...
  return <MasterDataContext.Provider value={contextValue}>{children}</MasterDataContext.Provider>;
}

const fallbackMasterDataContext: MasterDataContextType = {
  schools: [],
  academicYears: [],
  grades: [],
  transportRoutes: [],
...
export function useMasterData() {
  ...
  if (!context) {
    return fallbackMasterDataContext;
  }
  return context;
}
