...
export const ApplicationDetails: React.FC<ApplicationDetailsProps> = ({
  ...
  const handleEnrol = async () => {
    if (
      !confirm(
        'Are you sure you want to enrol this applicant as a student? This will create their student profile.',
      )
    )
      return;
    setActionLoading(true);
  ...
  const filteredLogs =
    app.admission_audit_logs?.filter(
      (log: any) => isStaff || publicActions.includes(log.action.toUpperCase()),
    ) || [];
  ...
  return (
    <div className={`${mode === 'page' ? 'p-6 max-w-5xl mx-auto' : 'h-full flex flex-col'}`}>
      ...
      <div
        className={`flex flex-col ${mode === 'page' ? 'md:flex-row' : ''} gap-6 ${mode === 'drawer' ? 'overflow-y-auto p-6 scrollbar-hide' : ''}`}
      >
        {/* Main Info */}
          ...
          <div className="bg-white rounded-2xl p-8 border border-gray-100 shadow-sm">
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="flex flex-col items-end gap-2">
                <span
                  className={`px-4 py-1 rounded-full text-sm font-bold uppercase tracking-wider bg-blue-50 text-blue-600 border border-blue-100`}
                >
                  ...
                  <span
                    className={`px-3 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${
                      app.applicant.login_status === 'APPROVED'
                        ...
                        : app.applicant.login_status === 'PENDING'
          ...
          <div className="bg-white rounded-2xl p-8 border border-gray-100 shadow-sm">
            ...
            <div className="space-y-6">
              {filteredLogs.map((log: any) => (
                <div
                  key={log.id}
                  className="relative pl-8 border-l-2 border-gray-100 last:border-0 pb-6 last:pb-0"
                >
        ...
        <div className="w-full md:w-80 space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            <AdmissionTimeline
              steps={getAdmissionTimelineSteps(app.status, app.admission_audit_logs)}
            />
          </div>
            ...
            <>
              <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-4">
                      {app.applicant?.login_status !== 'APPROVED' && (
                        <div className="p-4 bg-amber-50 rounded-xl border border-amber-100 text-xs text-amber-800">
                          <strong>Admission approved</strong>, but login access is still{' '}
                          <strong>{app.applicant?.login_status}</strong>. Approve login below to
                          enable enrollment.
                        </div>
                      )}
                      <button
                  ...
                  {AdmissionPermissions.canDecideLogin(permCtx) &&
                    app.applicant &&
                    app.applicant.login_status !== 'APPROVED' && (
                    ...
                    <>
                      ...
                      <div className="grid gap-2">
                          ...
                          hasPermission('admission.approve') && (
                            <div className="space-y-2 border-t pt-4">
                              ...
                              <div className="flex gap-2">
                                <button
                                  onClick={() =>
                                    handleVerifyFee('verified', remark || 'Payment verified')
                                  }
                                  disabled={actionLoading}
                                ...
                                <button
                                  onClick={() =>
                                    handleVerifyFee('correction', remark || 'Need correction')
                                  }
                                  disabled={actionLoading || !remark}
                        ...
                        {app.status === 'payment_verified' &&
                          hasPermission('admission.recommend') && (
                            <button
              ...
              <div className="bg-indigo-50 rounded-2xl p-6 border border-indigo-100">
                ...
                <p className="text-xs text-indigo-600 leading-relaxed">
                  {isStaff
                    ? 'These remarks are visible to all staff members but restricted for parents based on action type.'
                    : ''}
                </p>
              </div>
            </>
