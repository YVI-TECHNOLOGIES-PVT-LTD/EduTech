...
import {
  REGISTERED_WORKSPACE_LIST,
  ADMISSION_WORKSPACE_REGISTRY,
} from '../../core/AdmissionWorkspaceRegistry';
...
export function AdmissionOfficerDashboard() {
  ...
  const { progress: selectedProgress, isLoading: selectedProgressLoading } = useApplicationProgress(
  ...
  const handleWorkspaceChange = (workspaceId: string) => {
    switch (workspaceId) {
      ...
      case 'QUEUES':
        ...
        break;
      case 'DOCUMENTS':
        navigate('/app/admissions/verification');
        break;
      case 'INTERVIEWS':
        navigate('/app/admissions/interviews');
        break;
      case 'MERIT':
        navigate('/app/admissions/merit');
        break;
      case 'OFFERS':
        navigate('/app/admissions/offers');
        break;
      case 'FINANCE':
        navigate('/app/admissions/fees');
        break;
      case 'ENROLLMENT':
        navigate('/app/admissions/enrollment');
        break;
      case 'REPORTS':
        navigate('/app/admissions/reports');
        break;
      case 'SETTINGS':
  ...
  const applicant360View = useMemo(() => {
    ...
    return {
      ...
      crmLeadTemp: (selectedApplication.lead_temperature || 'warm') as any,
      crmLeadScore: selectedApplication.lead_score ?? selectedApplication.score ?? 0,
      documentChecklist: docs.map((d: any) => ({
        name: (d.document_type || d.type || '').replace(/_/g, ' ').toUpperCase(),
        verified: d.status === 'VERIFIED' || d.verify_status === 'verified',
      })),
      timelineNodes: [],
      ...
      examScore: selectedApplication.exam_score ?? 0,
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-xl font-black text-gray-900 uppercase tracking-wider flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-indigo-500 animate-pulse" /> Enterprise Admission Desk
            Console
          </h2>
          ...
          <div className="w-full max-w-6xl h-full bg-gray-50 p-6 shadow-2xl flex flex-col space-y-4 overflow-y-auto">
            <div className="flex items-center justify-between border-b pb-3 shrink-0">
              <div>
                ...
                <p className="text-xs text-gray-400 font-bold uppercase">
                  Track verification checklists, billing installments, scorecard panel details, and
                  SIS provisioning
                </p>
              </div>
              <Button
