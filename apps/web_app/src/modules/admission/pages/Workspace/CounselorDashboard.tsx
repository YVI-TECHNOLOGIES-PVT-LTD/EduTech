...
export function CounselorDashboard() {
  ...
  const [followupTab, setFollowupTab] =
    React.useState<(typeof FOLLOWUP_TABS)[number]['id']>('today');

  // Reset search query and tab on user/tenant change
  React.useEffect(() => {
    setQuery('');
    setFollowupTab('today');
  }, [user?.id, user?.school_id, setQuery]);
  ...
  const unassignedLeads = useMemo(() => leads.filter((l: any) => !isAssigned(l)), [leads]);
                    ...
                    >
                      <div className="min-w-0">
                        <p className="font-bold text-gray-900 dark:text-gray-100 text-xs">
                          {(lead as any).student_name || (lead as any).studentName}
                        </p>
                        <p className="text-[10px] text-gray-400 font-bold uppercase mt-0.5">
                          {(lead as any).inquiry_number ??
                            (lead as any).leadNumber ??
                            lead.id.slice(0, 8)}{' '}
                          ·{' '}
                          {(lead as any).grade_applied_for ?? (lead as any).gradeApplyingFor ?? '—'}
                        </p>
                      </div>
          ...
          {unassignedLeads.length > 0 && (
            <div className="space-y-3">
              ...
              {unassignedLeads.slice(0, 3).map((lead: any) => (
                <LeadCard key={lead.id} lead={lead as Lead} showAssign counselorId={counselorId} />
              ))}
            </div>
          )}
        </div>
          ...
          <div className="bg-white dark:bg-card border border-gray-150 dark:border-border/60 p-5 rounded-2xl shadow-sm space-y-4">
            ...
            <div className="divide-y divide-gray-100 text-xs max-h-[360px] overflow-y-auto">
              {followupItems.length === 0 ? (
                <p className="text-gray-400 py-4 text-center text-[10px]">
