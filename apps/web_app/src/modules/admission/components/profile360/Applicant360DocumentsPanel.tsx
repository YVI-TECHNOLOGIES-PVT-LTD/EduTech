...
export function Applicant360DocumentsPanel({
  applicationId,
  progress,
  readOnlyMode = false,
}: Applicant360DocumentsPanelProps) {
  ...
  const statusBadge = (status: string, verified: boolean) => {
    if (verified || status === 'VERIFIED') {
      return (
        <span className="text-[9px] font-black uppercase px-1.5 py-0.5 rounded bg-emerald-50 text-emerald-600">
          Verified
        </span>
      );
    }
    ...
    if (status === 'PENDING_VERIFICATION' || status === 'UPLOADED') {
      return (
        <span className="text-[9px] font-black uppercase px-1.5 py-0.5 rounded bg-amber-50 text-amber-600">
          Pending
        </span>
      );
    }
    return (
      <span className="text-[9px] font-black uppercase px-1.5 py-0.5 rounded bg-gray-100 text-gray-500">
        Missing
      </span>
    );
  };
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {checklistItems.map((item) => {
            const doc =
              documents.find((d: any) => item.documentId && d.id === item.documentId) ??
              documents.find((d: any) =>
                d.original_filename
                  ?.toLowerCase()
                  .includes(item.code.toLowerCase().replace(/_/g, ' ')),
              );
            ...
            return (
              <div
                key={item.code}
                className="flex items-center justify-between gap-2 p-3 border rounded-xl text-xs"
              >
                  ...
                  {docId && (
                    <>
                      ...
                      {canVerify && item.status !== 'VERIFIED' && (
                        <button
                          type="button"
                          onClick={() => verifyDocument({ documentId: docId })}
          ...
          {(['interview', 'exam', 'fees', 'verification'] as const).map((key) => (
            <div key={key} className="p-2 bg-gray-50 rounded-lg">
              <p className="text-[9px] font-bold text-gray-400 uppercase">
                {progress.sections[key].label}
              </p>
              <p className="text-xs font-black text-gray-700">
                {formatSectionStatus(progress.sections[key].status)}
              </p>
            </div>
          ))}
        </div>
