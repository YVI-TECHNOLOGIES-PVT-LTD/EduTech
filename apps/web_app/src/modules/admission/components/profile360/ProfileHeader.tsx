...
interface ProfileHeaderProps {
  applicant: Pick<
    Applicant360View,
    'name' | 'code' | 'email' | 'phone' | 'grade' | 'status' | 'candidateScore' | 'submittedAt'
  >;
}
...
export function ProfileHeader({ applicant }: ProfileHeaderProps) {
  ...
  const getStatusStyle = (status?: string) => {
    ...
    if (s.includes('REJECT') || s.includes('BREACH'))
      return 'bg-rose-50 text-rose-700 border-rose-200';
    if (s.includes('PENDING') || s.includes('CHECK'))
      return 'bg-amber-50 text-amber-700 border-amber-200';
    return 'bg-indigo-50 text-indigo-700 border-indigo-200';
  };

  const statusText = (applicant?.status || 'Submitted').replace(/_/g, ' ');
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-[10px] font-bold font-mono uppercase text-indigo-600 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded-lg">
                {applicant?.code || 'APP'}
              </span>
            </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-right">
            ...
            <span
              className={`px-2.5 py-1 rounded-full text-xs font-black uppercase border tracking-wider mt-0.5 inline-block ${getStatusStyle(applicant?.status)}`}
            >
              {statusText}
            </span>
          </div>
      ...
      <div className="pt-4 border-t border-gray-50 dark:border-border/10 flex flex-wrap items-center justify-between gap-4 text-xs font-medium text-gray-500">
        ...
        {applicant?.candidateScore !== undefined && (
          <div className="flex items-center gap-1">
            ...
            <span>
              Aggregated Admission Score:{' '}
              <strong className="text-indigo-600 font-black">{applicant.candidateScore}/100</strong>
            </span>
          </div>
        )}
      </div>
