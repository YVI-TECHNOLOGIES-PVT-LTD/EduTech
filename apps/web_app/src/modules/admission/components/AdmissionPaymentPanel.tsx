...
const GLASS_BASE =
  'backdrop-blur-xl bg-white/10 dark:bg-black/30 border border-white/20 dark:border-white/10 shadow-[0_8px_32px_0_rgba(31,38,135,0.07)] rounded-3xl';
    ...
    return (
      <div className={`${GLASS_BASE} p-10 text-center space-y-4`}>
        ...
        <p className="text-sm text-gray-500 max-w-sm mx-auto leading-relaxed">
          Your application is under review. Payment will be enabled after document verification and
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="p-8 space-y-4">
          <div className="space-y-1">
            {sortedFees.map((fee) => (
              <InvoiceRow
                key={fee.id}
                label={fee.snapshot_name}
                amount={fee.snapshot_amount}
                category={fee.snapshot_category}
            ...
            <div className="flex gap-3">
              ...
              <div className="hidden md:flex flex-col justify-center px-4 text-right">
                ...
                <p className="text-[10px] font-bold text-gray-900">
                  {app.payment_date ? new Date(app.payment_date).toLocaleDateString() : 'N/A'}
                </p>
              </div>
            </div>
          ) : (
      ...
      <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest text-center px-10 leading-relaxed">
        This invoice is generated based on a secure snapshot taken by the admission office. Any
        changes to institutional fee structures will not affect this established application.
      </p>
    </div>
  );
};
    ...
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <div className="flex gap-2">
          ...
          {isMandatory && (
            <span className="text-[7px] font-black text-blue-500 uppercase tracking-widest opacity-60">
              Mandatory
            </span>
          )}
        </div>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (proofType === 'link') {
      ...
      try {
        ...
        const {
          data: { publicUrl },
        } = supabase.storage.from('admissions').getPublicUrl(`payment_proofs/${fileName}`);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {selectedId && (
        ...
        >
          ...
          {mode === 'CARD' || mode === 'BANK_TRANSFER' ? (
            <div className="bg-card border border-border rounded-3xl p-6 shadow-xl space-y-6 text-card-foreground">
          ...
          <div className="pt-4 border-t border-white/5">
            <div className="flex flex-col md:flex-row gap-4">
              <div className="md:w-1/3 space-y-2">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 text-blue-500">
                  Proof Type
                </label>
                <select
                  value={proofType}
                ...
                >
                  <option value="link" className="bg-slate-900 border-none">
                    External Link
                  </option>
                  <option value="upload" className="bg-slate-900 border-none">
                    Upload Screenshot
                  </option>
                </select>
              </div>
      ...
      <Button
        ...
        className={`w-full rounded-2xl h-16 font-black uppercase tracking-widest text-[11px] shadow-2xl active:scale-[0.98] transition-all ${
          !selectedId
      ...
      >
        {loading || uploading
          ? uploading
            ? 'Uploading Proof...'
            : 'Processing...'
          : `Submit ${selectedLabel || 'Payment'} Details`}
