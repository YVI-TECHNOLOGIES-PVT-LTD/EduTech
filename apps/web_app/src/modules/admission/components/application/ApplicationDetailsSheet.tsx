...
export const ApplicationDetailsSheet: React.FC<ApplicationDetailsSheetProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'documents' | 'assessment' | 'decision' | 'payment'>('overview');
  ...
  const getStepStatus = (stepName: string) => {
    switch (stepName) {
      ...
      case 'decision':
        if (['submitted', 'documents_pending', 'assessment_pending'].includes(status)) return 'upcoming';
      ...
      case 'payment':
        if (['approved'].includes(status)) return payment?.payment_status === 'paid' ? 'completed' : 'current';
        if (['rejected', 'withdrawn'].includes(status)) return 'stopped';
        return 'upcoming';
      default:
        return 'upcoming';
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <SheetDescription className="text-xs text-slate-500 flex items-center gap-3 mt-1">
                ...
                <span>•</span>
                <span className="font-medium text-slate-700 dark:text-slate-300">{gradeName}</span>
                <span>•</span>
        ...
        <div className="px-5 pt-3 bg-white dark:bg-slate-900 border-b shrink-0">
          <Tabs value={activeTab} onValueChange={(v: any) => setActiveTab(v)} className="w-full">
            <TabsList className="grid grid-cols-5 h-9 bg-slate-100 dark:bg-slate-800 p-1 text-xs">
              ...
              <TabsTrigger value="assessment" className="text-xs font-semibold">Assessment</TabsTrigger>
              <TabsTrigger value="decision" className="text-xs font-semibold">Decision</TabsTrigger>
              <TabsTrigger value="payment" className="text-xs font-semibold">Payment</TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
                  ...
                  <div>
                    ...
                    <span className="font-semibold text-slate-800 dark:text-slate-200">{gradeName}</span>
                  </div>
                  <div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xs text-slate-500 mt-0.5">
                      <strong>{docsSummary.verified}</strong> of <strong>{docsSummary.total}</strong> documents verified ({docsSummary.pending} pending, {docsSummary.rejected} action needed)
                    </div>
                  </div>
                  <Button
                    size="sm"
              ...
              {docs.length > 0 ? (
                ...
                <div className="p-8 text-center bg-white dark:bg-slate-900 border border-dashed rounded-xl space-y-2">
                  ...
                  <div className="text-[11px] text-slate-400">
                    Required admission documents can be uploaded in the downstream Document Verification desk.
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="space-y-4">
              <div className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-slate-500 mt-0.5">
                    {assessment ? `Recorded on ${new Date(assessment.assessment_date).toLocaleDateString()}` : 'No entrance test recorded yet'}
                  </div>
                </div>
                <Button
                  size="sm"
              ...
              {assessment ? (
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                    ...
                    <div>
                      ...
                      <span className="text-base font-bold text-blue-600">
                        {assessment.percentage !== null && assessment.percentage !== undefined ? `${assessment.percentage}%` : '—'}
                      </span>
                    </div>
                    <div>
                ...
                <div className="p-8 text-center bg-white dark:bg-slate-900 border border-dashed rounded-xl space-y-2">
                  ...
                  <div className="text-[11px] text-slate-400">
                    Applicant can be scheduled for written, oral, or observation evaluation in the assessment desk.
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="space-y-4">
              <div className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-slate-500 mt-0.5">
                    {decision ? `Status: ${decision.decision_status}` : 'Pending committee / principal decision'}
                  </div>
                </div>
                <Button
                  size="sm"
              ...
              {decision ? (
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-3">
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
                    ...
                    <div>
                      ...
                      <span className="font-semibold text-slate-800 dark:text-slate-200">
                        {decision.scholarship_percentage ? `${decision.scholarship_percentage}%` : 'None'}
                      </span>
                    </div>
                ...
                <div className="p-8 text-center bg-white dark:bg-slate-900 border border-dashed rounded-xl space-y-2">
                  ...
                  <div className="text-[11px] text-slate-400">
                    Final committee decision (Approve, Waitlist, Reject) will be reflected once review concludes.
                  </div>
                </div>
              )}
            </div>
              ...
              {payment ? (
                ...
                <div className="p-8 text-center bg-white dark:bg-slate-900 border border-dashed rounded-xl space-y-3">
                  ...
                  <p className="text-[11px] text-slate-400 max-w-sm mx-auto">
                    Admission fee can be collected directly from this sheet or in the Front Office Fee desk.
                  </p>
                  <Button
                    size="sm"
                    onClick={() => setIsCollectFeeOpen(true)}
