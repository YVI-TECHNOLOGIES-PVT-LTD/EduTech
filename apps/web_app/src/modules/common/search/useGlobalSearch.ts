...
const NAV_RESULTS: SearchResultItem[] = [
  {
    ...
    module: 'navigation',
    href: '/app/front-office/dashboard',
  },
  {
    id: 'nav-parent-dashboard',
  ...
  { id: 'nav-students', label: 'Student List', module: 'students', href: '/app/people/students' },
  {
    id: 'nav-admissions',
  ...
  { id: 'nav-fees', label: 'Fee Payment', module: 'finance', href: '/app/front-office/fees' },
  ...
  const search = useCallback(async (query: string, category: string) => {
    ...
    try {
      ...
      const navMatches = NAV_RESULTS.filter(
        (r) => (category === 'all' || r.module === category) && r.label.toLowerCase().includes(q),
      );
      ...
      if (category === 'all' || category === 'admissions') {
        try {
          const res = await apiClient.get('/v1/admission/application', {
