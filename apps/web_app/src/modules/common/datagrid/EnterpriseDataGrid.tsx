...
export function EnterpriseDataGrid<T extends { id?: string | number }>({
  ...
  const groupByOptions = [
    ...
    ...columns.filter((c) => c.sortable !== false).map((c) => ({ value: c.key, label: c.header })),
  ];
  ...
  const isSomeSelected =
    grid.paginatedData.some((row) => row.id != null && grid.selectedRows[row.id]) && !isAllSelected;
  ...
  const renderRow = (row: T, idx: number) => {
    ...
    return (
      ...
      >
        {bulkActions && (
          <TableCell
            className="text-center w-12 p-0 border-r border-border/50"
                          ...
                          <span
                            ...
                            onMouseDown={(e) => {
                              ...
                              const onMove = (ev: MouseEvent) => {
                                grid.resizeColumn(col.key, startW + (ev.clientX - startX));
                              };
                              const onUp = () => {
