...
interface VersionTimelineProps {
    ...
    onCompare: (v1: QuestionItem, v2: QuestionItem) => void;
    ...
    return (
        <div className="space-y-4 bg-white dark:bg-card border border-gray-100 p-6 rounded-3xl shadow-premium-sm">
            <h4 className="text-sm font-black text-gray-900 flex items-center gap-1.5 border-b border-gray-50 pb-3">
                <History className="w-4.5 h-4.5 text-primary" /> Version History Timeline
            </h4>

            {versions.length <= 1 ? (
                <p className="text-xs text-gray-400 font-bold text-center py-6">No past versions found. Edits create immutable version nodes.</p>
            ) : (
