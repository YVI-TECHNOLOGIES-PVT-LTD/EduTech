...
import { useGetStudentsQuery } from '@/shared/api/student.api';
...
export function StudentListPage() {
  ...
  const { data, isLoading } = useGetStudentsQuery({ page, limit: 10, search, status: statusFilter });
  ...
  const columns: ColumnDefinition<any>[] = [
    { key: 'admission_no', header: 'Adm No' },
    {
      key: 'name',
      header: 'Full Name',
      render: (row: any) => `${row.first_name || row.firstName || ''} ${row.last_name || row.lastName || ''}`,
    },
    {
      key: 'grade',
      ...
      render: (row: any) => {
        ...
        return sec
          ? `Grade ${row.grade || 'N/A'} - Sec ${sec.section_id}`
          : `Grade ${row.grade || 'N/A'} - Unallocated`;
      },
    },
    {
      key: 'status',
  ...
  const mockStudents = (data as any)?.data || (Array.isArray(data) ? data : null) || [
  ...
  return (
    <div className="space-y-6 pb-6">
      ...
      {/* List Table */}
      <div>
        <DataTableFramework columns={columns} data={mockStudents} loading={isLoading} />
      </div>
    </div>
  );
}
