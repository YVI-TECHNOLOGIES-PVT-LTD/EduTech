...
type ReportType =
  'collections' | 'outstanding' | 'aging' | 'cash-closing' | 'payment-mode' | 'class-summary';
...
export const FinanceReports = () => {
  ...
  const exportCSV = () => {
    ...
    const rows =
      data.data || data.buckets
        ? Object.entries(data.buckets || {}).map(([k, v]) => ({ range: k, amount: v }))
        : [];
        ...
        <ResponsiveContainer width="100%" height={260}>
          <PieChart>
            <Pie
              ...
              cy="50%"
              outerRadius={100}
              label={({ mode, percent = 0 }) => `${mode} ${(percent * 100).toFixed(0)}%`}
              labelLine={false}
            >
  ...
  const renderTable = () => {
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            {Object.entries(data.byMode || {}).map(([mode, total]: any) => (
              <div
                key={mode}
                className="flex justify-between text-sm p-2.5 rounded-lg bg-slate-50 dark:bg-white/5"
              >
    ...
    if (!rows.length)
      return (
        <p className="text-center text-slate-400 py-8 text-sm">No data for selected parameters</p>
      );
          ...
          <thead>
            <tr className="border-b border-slate-200 dark:border-white/10 bg-slate-50/50 dark:bg-white/5">
              {keys.map((k) => (
                <th
                  key={k}
                  className="px-3 py-2 text-left text-xs font-bold text-slate-500 uppercase"
                >
                  {k.replace(/_/g, ' ')}
                </th>
              ))}
              ...
              <tr key={i} className="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors">
                {keys.map((k) => (
                  <td key={k} className="px-3 py-2 text-xs text-slate-600 dark:text-slate-400">
                    {typeof row[k] === 'number' && k.includes('amount')
    ...
    <div className="p-6 md:p-8 space-y-6 text-slate-800 dark:text-slate-100">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black tracking-tight flex items-center gap-3">
            <BarChart2 className="w-6 h-6 text-violet-500" /> Finance Reports
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-4">
              ...
              <>
                <div>
                  ...
                  <input
                    ...
                    className="px-3 py-1.5 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl outline-none"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-bold uppercase text-slate-400 mb-1">
          ...
          {/* Chart */}
          {data && renderChart() && <div className={`${GLASS} p-5`}>{renderChart()}</div>}
