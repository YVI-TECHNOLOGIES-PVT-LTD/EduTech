...
export const DemandManagement = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      if (genMode === 'single') {
        ...
        if (genMode === 'custom')
          payload.student_ids = genForm.student_ids
            .split(',')
            .map((s) => s.trim())
            .filter(Boolean);
  ...
  const filtered = demands.filter((d) => {
    ...
    const matchSearch =
      !search || name.toLowerCase().includes(search.toLowerCase()) || d.demand_no?.includes(search);
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-400">
                        {d.fee_structure?.name || '—'}
                      </td>
                      <td className="px-4 py-3 font-semibold">
                        ₹{Number(d.amount).toLocaleString('en-IN')}
                      </td>
                      <td className="px-4 py-3 font-bold text-orange-600 dark:text-orange-400">
                        ₹{Number(d.balance_amount).toLocaleString('en-IN')}
                      </td>
                      <td className="px-4 py-3 text-slate-500">{d.due_date}</td>
                      <td className="px-4 py-3">
                        <span
                          className={`px-2 py-1 rounded-full text-[11px] font-bold ${STATUS_COLORS[d.status] || ''}`}
                        >
                          {d.status}
                        </span>
                      </td>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className={`${GLASS} w-full max-w-lg p-6 space-y-5`}>
            ...
            <div className="grid grid-cols-4 gap-2">
              {(
                [
                  ['single', 'Single Student', Users],
                  ['class', 'Entire Class', Layers],
                  ['section', 'Section', Layers],
                  ['custom', 'Custom', FileText],
                ] as any[]
              ).map(([m, label, Icon]) => (
                <button
                  key={m}
            ...
            <div className="space-y-3">
              ...
              {genMode === 'class' && (
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      setGenForm((f) => ({ ...f, class_id: e.target.value }));
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setGenForm((f) => ({ ...f, section_id: e.target.value }))}
                      className="w-full px-3 py-2 text-sm border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-800 rounded-xl outline-none"
                    >
                      <option value="">Select</option>
                      {sections.map((s: any) => (
              ...
              {genMode === 'custom' && (
                <div>
                  ...
                  <textarea
                    value={genForm.student_ids}
                    onChange={(e) => setGenForm((f) => ({ ...f, student_ids: e.target.value }))}
                    rows={3}
                    className="w-full px-3 py-2 text-sm border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-800 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500/30 resize-none"
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setGenForm((f) => ({ ...f, fee_structure_id: e.target.value }))}
                  className="w-full px-3 py-2 text-sm border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-800 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500/30"
                >
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                className="flex-1 py-2 rounded-xl border border-slate-200 dark:border-white/10 text-sm font-semibold text-slate-600 hover:bg-slate-50 dark:hover:bg-white/5 transition-all"
              >
                Cancel
              </button>
              <button
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className={`${GLASS} w-full max-w-sm p-6 space-y-4`}>
            ...
            <p className="text-sm text-slate-500">
              This will cancel the billing demand. This cannot be undone.
            </p>
            <textarea
              value={cancelReason}
              onChange={(e) => setCancelReason(e.target.value)}
              rows={3}
              className="w-full px-3 py-2 text-sm border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-800 rounded-xl outline-none resize-none"
              placeholder="Reason for cancellation (optional)"
            />
            <div className="flex gap-3">
              <button
                onClick={() => setCancelId(null)}
                className="flex-1 py-2 rounded-xl border border-slate-200 dark:border-white/10 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-all"
              >
                Back
              </button>
              <button
