...
export const Waivers = () => {
  ...
  const handleApply = async () => {
    if (!demandId || !amount || Number(amount) <= 0)
      return toast.error('Enter demand ID and a valid amount');
    setSaving(true);
    try {
    ...
    <div className="p-6 md:p-8 space-y-6 text-slate-800 dark:text-slate-100">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Apply fee concessions — posts a credit to the student ledger
        </p>
      </div>
      ...
      <div className={`${GLASS} p-6 max-w-xl space-y-5`}>
        ...
        {demand && (
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4 space-y-1 text-sm">
            ...
            <p className="text-slate-600 dark:text-slate-400">
              Student:{' '}
              <strong>
                {demand.student?.full_name || demand.application?.applicant_name || '—'}
              </strong>
            </p>
            <p className="text-slate-600 dark:text-slate-400">
              Total: <strong>₹{Number(demand.amount).toLocaleString('en-IN')}</strong>
            </p>
            <p className="text-red-600 dark:text-red-400 font-bold">
              Outstanding: ₹{Number(demand.balance_amount).toLocaleString('en-IN')}
            </p>
            <p className="text-slate-400 text-xs">
              Due: {demand.due_date} · Status: {demand.status}
            </p>
          </div>
        )}
        ...
        <button
          onClick={handleApply}
          disabled={saving || !demandId || !amount}
        ...
        <p className="text-xs text-slate-400 text-center">
          This action posts a WAIVER credit to the student ledger and reduces the outstanding
          balance. This is permanent and audited.
        </p>
      </div>
    </div>
  );
