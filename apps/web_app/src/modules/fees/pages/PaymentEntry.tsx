...
const GLASS_BASE =
  'backdrop-blur-xl bg-white/70 dark:bg-black/50 border border-white/20 dark:border-white/10 shadow-[0_8px_32px_0_rgba(31,38,135,0.04)] rounded-3xl p-6 transition-all duration-300 hover:shadow-lg';
...
export const PaymentEntry = () => {
  ...
  const [mode, setMode] = useState<
    'Cash' | 'UPI' | 'Card' | 'Cheque' | 'Bank_Transfer' | 'Online_Gateway'
  >('Cash');
        ...
        <div className={`${GLASS_BASE} text-center space-y-6 p-10 border border-emerald-500/20`}>
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              RCPT Transaction ID: {activeReceipt.receipt_no}
            </p>
          </div>
    ...
    <div className="max-w-7xl mx-auto p-8 space-y-8 text-slate-800 dark:text-slate-100">
      ...
      <div>
        ...
        <p className="text-sm font-medium text-slate-500 mt-1">
          Process counter payments and manage pending collections
        </p>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="relative w-full md:w-96">
          ...
          <input
            className="w-full bg-card border border-border text-foreground pl-10 pr-4 py-3 rounded-2xl outline-none text-xs"
        ...
        <div className="flex items-center gap-3">
          ...
          <select
            className="bg-card border border-border text-foreground px-4 py-2.5 rounded-2xl text-xs outline-none"
            value={statusFilter}
        ...
        <div className={`${GLASS_BASE} lg:col-span-2 space-y-4`}>
          ...
          <div className="space-y-4">
            {filteredDemands.map((d) => (
              <div
                key={d.id}
                className="p-4 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-3xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4"
              >
                ...
                <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end border-t md:border-0 border-slate-200/50 pt-3 md:pt-0">
                  <div className="text-right">
                    ...
                    <p className="text-lg font-black text-rose-600">
                      ₹{Number(d.balance_amount).toLocaleString()}
                    </p>
                  </div>
                  <button
                    onClick={() => {
            ...
            {filteredDemands.length === 0 && !loading && (
              <div className="text-center py-12 opacity-50">
                ...
                <p className="text-xs font-bold uppercase tracking-widest">
                  All demands settled in this queue
                </p>
              </div>
            )}
          </div>
        ...
        <div className={`${GLASS_BASE}`}>
          ...
          {activeDemand ? (
            ...
            >
              <div className="p-4 bg-indigo-500/10 border border-indigo-500/20 rounded-2xl text-xs">
                <p className="font-bold text-indigo-700">
                  Billing target: {activeDemand.demand_no}
                </p>
                <p className="text-[10px] text-indigo-600 mt-1">
                  Owner:{' '}
                  {activeDemand.student?.full_name || activeDemand.application?.applicant_name}
                </p>
              </div>
            ...
            <div className="text-center py-24 opacity-40">
              ...
              <p className="text-xs font-bold uppercase tracking-widest">
                Select an item from the queue to start billing collection
              </p>
            </div>
          )}
        </div>
