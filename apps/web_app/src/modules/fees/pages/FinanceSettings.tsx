    ...
    <div className="p-6 md:p-8 space-y-6 text-slate-800 dark:text-slate-100">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Configure global financial engine parameters
          </p>
        </div>
      </div>
      ...
      <div className={`${GLASS} p-6 max-w-2xl space-y-6`}>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">
              Demand Number Prefix
            </label>
            <input
              value={settings.demand_prefix || ''}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">
              Default Payment Window (Days)
            </label>
            <input
              type="number"
              value={settings.default_payment_window_days ?? 30}
              onChange={(e) =>
                setSettings((s: any) => ({
                  ...s,
                  default_payment_window_days: parseInt(e.target.value) || 30,
                }))
              }
              className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500/30"
            />
          ...
          <div>
            ...
            <input
              value={settings.school_year_label || ''}
              onChange={(e) =>
                setSettings((s: any) => ({ ...s, school_year_label: e.target.value }))
              }
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400">
                Automatically calculate and post penalty for overdue demands
              </p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  setSettings((s: any) => ({ ...s, late_fee_enabled: e.target.checked }))
                }
            ...
            <div className="grid grid-cols-2 gap-4 p-4 bg-slate-50 dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/5">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSettings((s: any) => ({ ...s, grace_days: parseInt(e.target.value) || 0 }))
                  }
                  className="w-full px-3 py-2 text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl outline-none"
                />
              </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            placeholder="Thank you for your payment. Keep this copy for audit purposes."
        ...
        <div className="flex gap-3">
          <button
            ...
            className="px-4 py-2 border border-slate-200 dark:border-white/10 rounded-xl text-sm font-semibold hover:bg-slate-50 dark:hover:bg-white/5 transition-all"
          >
            Reset
          </button>
          <button
