...
export const ReceiptCenter = () => {
  ...
  const handlePrint = (receipt: any) => {
    ...
    win.document.write(`
            ...
            <style>body{font-family:'JetBrains Mono',monospace;padding:24px;font-size:13px}
            ...
            ${
              student
                ? `<div class="row"><span>Student:</span><span>${student.full_name}</span></div>
  ...
  const filtered = receipts.filter((r) => {
    ...
    return (
      !search || name.toLowerCase().includes(search.toLowerCase()) || r.receipt_no?.includes(search)
    );
  });
    ...
    <div className="p-6 md:p-8 space-y-6 text-slate-800 dark:text-slate-100">
      ...
      <div className={`${GLASS} p-4 flex flex-wrap gap-3`}>
        ...
        >
          ...
          {['ORIGINAL', 'REPRINT', 'DUPLICATE', 'CANCELLED'].map((t) => (
        ...
        <button
          ...
          className="px-4 py-2 bg-emerald-500 text-white text-sm font-semibold rounded-xl hover:bg-emerald-600 transition-all"
        >
          Apply
        </button>
        <button
          onClick={fetchReceipts}
          className="p-2 bg-slate-100 dark:bg-white/5 rounded-xl hover:bg-slate-200 transition-all"
        >
                ...
                filtered.map((r) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`px-2 py-1 rounded-full text-[11px] font-bold ${TYPE_COLORS[r.receipt_type] || ''}`}
                        >
                          {r.receipt_type}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="p-1.5 text-slate-500 hover:bg-slate-100 dark:hover:bg-white/10 rounded-lg transition-all"
                            title="Print"
                          >
                          ...
                          >
                            <RefreshCw
                              className={`w-4 h-4 ${reprinting === r.id ? 'animate-spin' : ''}`}
                            />
                          </button>
                        </div>
                      </td>
          ...
          <div className={`${GLASS} w-full max-w-md p-6 space-y-4`}>
            ...
            <div className="space-y-2 text-sm">
              {[
                ...
                <div
                  key={k}
                  className="flex justify-between border-b border-slate-100 dark:border-white/10 py-1.5"
                >
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 py-2 bg-slate-100 dark:bg-white/10 rounded-xl text-sm font-semibold hover:bg-slate-200 transition-all"
              >
                <Printer className="w-4 h-4" /> Print
              </button>
              <button
                onClick={() => {
                  handleReprint(viewing.id);
                  setViewing(null);
                }}
