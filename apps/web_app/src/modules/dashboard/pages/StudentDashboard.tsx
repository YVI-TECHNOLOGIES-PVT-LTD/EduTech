...
function StudentDashboardInner() {
  ...
  const announcements =
    engineNotifications && engineNotifications.length > 0
      ? engineNotifications.map((n: any) => ({
          id: n.id,
          title: n.title,
          body: n.message,
      ...
      : [
          {
            ...
            body: 'All students must report to auditorium by 3 PM today.',
            time: '2h ago',
            priority: 'high',
          },
          {
            ...
            body: 'Books issued in April must be returned by this Friday.',
            time: '1d ago',
            priority: 'medium',
          },
          {
            ...
            body: 'Register for track and field events before June 30.',
            time: '2d ago',
            priority: 'low',
          },
        ];
  ...
  const quickActions = [
    ...
    },
    {
      icon: GraduationCap,
      label: 'My Results',
      href: '/app/student/exams/dashboard',
        ...
        <div className="bg-white dark:bg-card border border-border/40 rounded-3xl p-6 shadow-premium-sm">
          ...
          <div className="space-y-4">
            {announcements.map((ann: any) => (
              <div
                key={ann.id}
                className={`p-4 rounded-2xl border-l-2 bg-gray-50/50 dark:bg-muted/10 ${
                  ann.priority === 'high'
                    ? 'border-red-500'
                    : ann.priority === 'medium'
                      ? 'border-amber-500'
              ...
              >
                ...
                <p className="text-[9px] text-muted-foreground/60 font-black uppercase mt-2">
                  {ann.time}
                </p>
              </div>
            ))}
    ...
    >
      <div className="space-y-6 lg:space-y-8">
        ...
        <div className="bg-white dark:bg-card border border-border/40 rounded-3xl p-6 shadow-premium-sm">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {quickActions.map((action, i) => (
              ...
              >
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center ${action.color} group-hover:scale-110 transition-transform mb-3`}
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          ...
          <div
            className={`p-6 rounded-3xl border ${attendancePercent < 75 ? 'bg-red-500/5 border-red-500/10' : 'bg-emerald-500/5 border-emerald-500/10'}`}
          >
            ...
            <p
              className={`text-xs font-semibold mt-2 ${attendancePercent < 75 ? 'text-red-500' : 'text-emerald-500'}`}
            >
              {attendancePercent < 75
                ? '⚠️ Attendance is below the mandatory 75% threshold.'
                : '✓ Your attendance criteria is fully matched.'}
            </p>
            <div className="mt-4 h-2 bg-gray-200/50 dark:bg-muted/20 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full ${attendancePercent < 75 ? 'bg-red-500' : 'bg-emerald-500'}`}
