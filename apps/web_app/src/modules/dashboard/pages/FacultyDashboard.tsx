...
const FacultyDashboardInner = () => {
  ...
  const kpis = [
    {
      label: 'Classes Today',
      value: getKPIValue('faculty.kpi.classes_today', stats?.classes_today || 0),
      icon: Clock,
      color: 'text-indigo-500 bg-indigo-500/10 border-indigo-500/20',
    },
    {
      label: 'My Sections',
      value: getKPIValue('faculty.kpi.my_sections', stats?.sections_count || 0),
      icon: Users,
      color: 'text-emerald-500 bg-emerald-500/10 border-emerald-500/20',
    },
    {
      label: 'Pending Works',
      value: getKPIValue('faculty.kpi.pending_works', stats?.pending_assignments || 0),
      icon: BookOpen,
  ...
  const operations = [
    ...
    },
    {
      label: 'Enter Marks',
      icon: Award,
      link: '/app/faculty/exams/marks-entry',
      desc: 'Exam grading lists',
      color: 'bg-emerald-500/10 text-emerald-500',
    },
    {
      label: 'Class Work',
      icon: BookOpen,
      link: '/app/academic/assignments',
        ...
        <div className="bg-white dark:bg-card border border-border/40 rounded-3xl p-6 shadow-premium-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {operations.map((action, i) => (
              ...
              >
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center border border-border/40 shadow-premium-sm ${action.color} group-hover:bg-primary group-hover:text-primary-foreground group-hover:border-transparent transition-colors`}
                >
        ...
        <div className="bg-white dark:bg-card border border-border/40 rounded-3xl p-6 shadow-premium-sm">
          ...
          <div className="text-center py-10 border border-dashed border-border/60 rounded-2xl bg-gray-50/30 dark:bg-muted/5">
            ...
            <p className="text-xs font-bold text-muted-foreground italic">
              Contact your section coordinator for active class details.
            </p>
          </div>
        </div>
      </div>
