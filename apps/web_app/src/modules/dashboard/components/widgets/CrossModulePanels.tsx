...
const CrossModuleTile = memo<CrossModuleTileProps>(
  ({ label, value, subtext, icon: Icon, link, accentClass, alertCount }) => (
    ...
    >
      <div
        className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border ${accentClass}`}
      >
...
export const AdminCrossModulePanel = memo(() => {
  ...
  const pendingFinance = tasks.filter(
    (t: any) => t.entityType?.toLowerCase() === 'fee' && t.status === 'pending',
  ).length;
  const pendingAdmissions = tasks.filter(
    (t: any) => t.entityType?.toLowerCase() === 'admission' && t.status === 'pending',
  ).length;
  ...
  const tiles: CrossModuleTileProps[] = [
    ...
    },
    {
      ...
      subtext: `${tasks.filter((t: any) => t.priority === 'urgent').length} urgent`,
      icon: AlertCircle,
      link: '/app/settings',
      accentClass:
        'bg-rose-50 text-rose-600 border-rose-100 dark:bg-rose-900/10 dark:text-rose-400 dark:border-rose-900/20',
      alertCount: tasks.filter((t: any) => t.priority === 'urgent').length,
    },
  ];
...
export const PrincipalCrossModulePanel = memo(() => {
  ...
  const pendingApprovals = tasks.filter(
    (t: any) => t.status === 'pending' && t.priority !== 'low',
  ).length;
...
export const AdmissionsCrossModulePanel = memo(() => {
  ...
  const pendingFees = tasks.filter(
    (t: any) => t.entityType?.toLowerCase() === 'fee' && t.status === 'pending',
  ).length;
