...
const StatusDot: React.FC<{ status: 'ok' | 'warn' | 'error'; label: string }> = ({
  status,
  label,
}) => {
  const colors = {
...
export const HealthPanel: React.FC = () => {
  ...
  const lastSyncLabel = lastSync > 0 ? new Date(lastSync).toLocaleTimeString() : 'Not yet synced';
  ...
  return (
    <div className="bg-white dark:bg-card border border-border/40 rounded-3xl shadow-premium-sm overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <span
            className={`w-2 h-2 rounded-full ${engineStatus === 'ok' ? 'bg-emerald-500' : engineStatus === 'warn' ? 'bg-amber-500' : 'bg-rose-500'} animate-pulse`}
          ></span>
          {expanded ? (
        ...
        <div className="px-6 pb-6 space-y-5 border-t border-border/30">
          ...
          <div className="pt-4 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <StatusDot
                status={engineStatus}
                label={`Dashboard Engine: ${engineStatus === 'ok' ? 'Online' : engineStatus === 'warn' ? 'Loading' : 'Error'}`}
              />
              <StatusDot
                status={cacheStatus}
                label={`React Query Cache: ${failedQueries > 0 ? `${failedQueries} failed` : 'Healthy'}`}
              />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              {[
                ...
                <div
                  key={s.label}
                  className="bg-gray-50 dark:bg-card/50 rounded-xl p-2 border border-border/30"
                >
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1.5">
              {[
                ...
                {
                  key: 'Pending Tasks',
                  val: String(tasks.filter((t: any) => t.status === 'pending').length),
                },
                {
                  key: 'Unread Notifications',
                  val: String(notifications.filter((n: any) => !n.read).length),
                },
              ].map((row) => (
                <div
                  key={row.key}
                  className="flex items-center justify-between text-[11px] py-1 border-b border-border/20 last:border-0"
                >
                  <span className="text-muted-foreground font-semibold">{row.key}</span>
                  <span className="font-black text-gray-900 dark:text-white text-right max-w-[60%] truncate">
                    {row.val}
                  </span>
                </div>
              ))}
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <span
                  key={flag}
                  className="px-2.5 py-1 rounded-full text-[9px] font-black bg-emerald-50 text-emerald-600 border border-emerald-100 dark:bg-emerald-900/10 dark:text-emerald-400 dark:border-emerald-900/20 uppercase tracking-wide"
                >
                  ✓ {flag}
                </span>
              ))}
