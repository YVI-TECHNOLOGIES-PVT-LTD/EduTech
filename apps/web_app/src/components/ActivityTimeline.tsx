...
export const ActivityTimeline = () => {
  ...
  const events = activities.map((act: any) => {
    ...
    if (titleL.includes('admission') || titleL.includes('applied') || titleL.includes('enrol')) {
      ...
      titleL.includes('message') ||
      titleL.includes('notice') ||
      titleL.includes('announc')
    ) {
      color = 'rose';
      icon = 'MessageSquare';
    }
      ...
      <AnimatePresence>
        {events.map((event: any, idx: number) => {
          ...
          return (
            ...
            >
              <div
                className={`w-10 h-10 rounded-full border flex items-center justify-center relative z-10 transition-all group-hover:scale-110 ${color}`}
              >
